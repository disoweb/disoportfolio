...
- June 26, 2025. Fixed critical authentication session management issue that was causing 401 "Authentication required" errors after successful login: updated isAuthenticated middleware to properly await user data retrieval, improved session configuration with saveUninitialized: false, and added comprehensive debugging logs that confirmed session persistence is now working correctly with order creation success